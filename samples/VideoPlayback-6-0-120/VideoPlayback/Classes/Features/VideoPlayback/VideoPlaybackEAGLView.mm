...
namespace {
    ...
    int list_version1[5] = { 13, 9, 10, 11, 12 }; // codepath
    int list_version2[1] = { 14 }; // kingsley
    ...
    for (int i = 0; i < kNumVideoTargets; ++i) {
        ...
        switch (i) {
            ...
            case 8:
                ...
                break;
            case 9:
                filename = @"iceland_plane_0.mp4";
                break;
            case 10:
                filename = @"iceland_plane_1.mp4";
                break;
            case 11:
                filename = @"iceland_plane_2.mp4";
                break;
            case 12:
                filename = @"iceland_plane_3.mp4";
                break;
            case 13:
                filename = @"iceland_plane_first.mp4";
                break;
            case 14:
                filename = @"iceland_video.mp4";
                break;
            default:
                filename = @"18_dl_fan.mp4";
                break;
...
- (void)renderFrameWithState:(const Vuforia::State &)state projectMatrix:(Vuforia::Matrix44F &)projectionMatrix
{
    ...
    for (int i = 0; i < numActiveTrackables; ++i) {
        ...
        if(imageTarget.getId() == 1) {
            ...
            NSDictionary *dict2 = [NSDictionary dictionaryWithObject:@"Untitled" forKey:@"name"];
            [[NSNotificationCenter defaultCenter] postNotificationName:@"scanOn_name"
                                                                object:nil
                                                              userInfo:dict2];
        ...
        if(imageTarget.getId() == 2) {
            // DL: INSERT DELEGATE HERE scanned(TRUE)
            // DL: TEST NSNOTIFICATIONCENTER!!! (Post)
            arrayLength = sizeof(list_version2)/sizeof(list_version2[0]);
            ...
            NSDictionary *dict2 = [NSDictionary dictionaryWithObject:@"Skógafoss, Iceland" forKey:@"name"];
            [[NSNotificationCenter defaultCenter] postNotificationName:@"scanOn_name"
                                                                object:nil
                                                              userInfo:dict2];
            ...
            //printf("Why is this crashing? current_index: %u\n", current_index);
            playerIndex = list_version2[current_index];
        } // fan
        
        if(imageTarget.getId() == 8){
            // DL: INSERT DELEGATE HERE scanned(TRUE)
            // DL: TEST NSNOTIFICATIONCENTER!!! (Post)
            ...
            NSDictionary *dict2 = [NSDictionary dictionaryWithObject:@"FAN-TASTIC!" forKey:@"name"];
            [[NSNotificationCenter defaultCenter] postNotificationName:@"scanOn_name"
                                                                object:nil
                                                              userInfo:dict2];
